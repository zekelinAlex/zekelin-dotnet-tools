...
The first row of the section is a read-only version indicator: `txc <version>` (with `txc-mcp <version>` as its description when the MCP server is installed too), read from `dotnet tool list --global`. It refreshes automatically after every Reinstall button finishes (click it to refresh on demand), so you can always tell whether a `0.0.0-local.*` dev build or a nuget.org release is currently active.

| Button | What it does |
| --- | --- |
| **Reinstall Local** | Writes the embedded script to a temp `.ps1` and runs `pwsh -NoProfile -ExecutionPolicy Bypass -File <temp> -RepoRoot "<workspace>"`. Streams output live; temp file is cleaned up afterwards. Works whether or not `<repo>/scripts/reinstall-local.ps1` exists on disk. Also uninstalls the `TALXIS.DevKit.Templates.Dataverse` template package and deletes every `*.nupkg` from `~/.templateengine/packages`, so txc reinstalls templates fresh on its next run. |
| **Reinstall Local (with MCP)** | Same as above with `-IncludeMcp` appended. |
| **Reinstall from NuGet** | Uninstalls the global `TALXIS.CLI` tool (typically a `0.0.0-local.*` dev build) and installs the latest published release from nuget.org (`dotnet tool install --global TALXIS.CLI --source https://api.nuget.org/v3/index.json`, so the local `C:\NuGetLocal` feed can't interfere). If `TALXIS.CLI.MCP` is currently installed, it's reinstalled from nuget.org too so both stay on the same version; if it isn't, it's left alone. Then the `TALXIS.DevKit.Templates.Dataverse` `dotnet new` template package gets the same treatment: uninstall + wipe of `~/.templateengine/packages` `*.nupkg`, resolve the latest release on nuget.org (`dotnet package search`), and `dotnet new install <id>::<latest>` pinned explicitly so a locally packed `99.0.0` or the local feed can't outrank it. |
