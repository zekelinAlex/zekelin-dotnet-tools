...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('dotnet-cleanup.gitWipe', (uri: vscode.Uri) => {
      return gitWipe(uri, outputChannel);
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('dotnet-cleanup.platformMetadataInstallNugets', () => {
      return platformMetadataInstallNugets(outputChannel);
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('dotnet-cleanup.toolsCliReinstallNuget', () => {
      return toolsCliReinstallNuget(outputChannel);
    })
  );
  context.subscriptions.push(
    vscode.commands.registerCommand('dotnet-cleanup.toolsCliRefreshVersions', () => toolsCliRefreshVersions())
  );
  context.subscriptions.push(
...
function setupPlatformMetadataContext(context: vscode.ExtensionContext) {
  const update = () => {
    const folders = vscode.workspace.workspaceFolders || [];
